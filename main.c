...
char* getRealText(debug_menu_entry* entry, char* str) {
	...
	if (entry->entry_type == BOOLEAN_F) {
		BYTE* num = entry->data;
		sprintf(str, "%s: %s", entry->text, *num ? "1" : "0");
...
if (entry->entry_type == BOOLEAN_NUM) {
...
typedef  (__fastcall* sub_55E7A0_ptr)(DWORD* this);
sub_55E7A0_ptr sub_55E7A0 = 0x0055E7A0;
...
uint8_t __stdcall slf_create_mission_replay_menu_entry(slf* function, void* unk)
{
	...
	void* res = add_debug_menu_entry(mission_replay_menu, &entry);
...
void menu_setup(int game_state, int keyboard) {
	...
	if (is_menu_key_pressed(MENU_TOGGLE, keyboard) && (game_state == 6 || game_state == 7)) {
		...
		if (game_flags_menu->used_slots == 0) {
			...
			debug_menu_entry test4 = { "Test Flag4", BOOLEAN_E,  0x895884 };
			add_debug_menu_entry(game_flags_menu, &test4);
			debug_menu_entry test5 = { "Test Flag5", BOOLEAN_E,  0x9563A8 };
			add_debug_menu_entry(game_flags_menu, &test5);
			debug_menu_entry obbnode = { "obbnode", BOOLEAN_E,  0x956880 };
			add_debug_menu_entry(game_flags_menu, &obbnode);
			debug_menu_entry obbidx = { "obbidx", BOOLEAN_E,  0x956790 };
			add_debug_menu_entry(game_flags_menu, &obbidx);
			debug_menu_entry obbarry = { "obbarry", BOOLEAN_E,  0x9563A8 };
			add_debug_menu_entry(game_flags_menu, &obbarry);
			debug_menu_entry obbflags = { "obbflags", BOOLEAN_E,  0x956964 };
			add_debug_menu_entry(game_flags_menu, &obbflags);
			debug_menu_entry aabb = { "aabb", BOOLEAN_E,  0x956438 };
			add_debug_menu_entry(game_flags_menu, &aabb);
			debug_menu_entry district = { "district", BOOLEAN_E,  0x9566C8 };
			add_debug_menu_entry(game_flags_menu, &district);
			debug_menu_entry sky_day = { "sky_day", BOOLEAN_E,  0x956388 };
			add_debug_menu_entry(game_flags_menu, &sky_day);
			debug_menu_entry sky_night = { "sky_night", BOOLEAN_E,  0x95638C };
			add_debug_menu_entry(game_flags_menu, &sky_night);
			debug_menu_entry sky_rainy = { "sky_rainy", BOOLEAN_E,  0x956390 };
			add_debug_menu_entry(game_flags_menu, &sky_rainy);
			debug_menu_entry sky_sunset = { "sky_sunset", BOOLEAN_E,  0x956394 };
			add_debug_menu_entry(game_flags_menu, &sky_sunset);
...
void menu_input_handler(int keyboard, int SCROLL_SPEED) {
	if (is_menu_key_clicked(MENU_DOWN, keyboard)) {
		...
		game_menu->handler(&game_menu->entries[game_menu->window_start + game_menu->cur_index], ENTER);
	}
...
void install_patches() {
	...
	*/
	...
	WriteDWORD(0x0089C710, slf_create_mission_replay_menu_entry, "hooking create_mission_replay_menu_entry");
...
void handle_distriction_variants_entry(debug_menu_entry* entry, custom_key_type key_type) {
...
void setup_debug_menu() {
	...
	district_variants_menu = create_menu("District variants", goto_start_debug, handle_distriction_variants_entry, 15);
	...
	progression_menu = create_menu("Progression", goto_start_debug, handle_progression_entry, 10);
